...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        tc = orm.SubtitleVersion.objects.filter(text_change__gt=1)
        affected = 0
        for x in tc:
            x.text_change = 1
            affected += 1
            if x.time_change > 1:
                x.time_change = 1
            x.save()
        time_changed = orm.SubtitleVersion.objects.filter(time_change__gt=1)
        for x in time_changed:
            x.time_change = 1
            x.save()
            affected += 1
        print "Truncated %s video versions" % affected
