...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print "Will set all null subversion.user models to AnonymousUser"
        pks = [x[0] for x in orm.SubtitleVersion.objects.filter(user__isnull=True).values_list("pk")]
        print "Will add the anon user to ids\n %s" % pks
        affected = orm.SubtitleVersion.objects.filter(user__isnull=True).update(user=settings.ANONYMOUS_USER_ID)
        print "will affect %s versions" % affected
