...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for v in orm.SubtitleVersion.objects.all():
            print v.pk
            
            if not v.user or not v.user.changes_notification:
                continue
            
            try:
                orm.StopNotification.objects.get(user=v.user, video=v.language.video)
                continue
            except models.ObjectDoesNotExist:
                pass
            
            v.language.followers.add(v.user)
            v.language.video.followers.add(v.user)
