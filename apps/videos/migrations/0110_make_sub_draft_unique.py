...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        count = orm.SubtitleDraft.objects.count()
        i = 0
        for d in orm.SubtitleDraft.objects.all():
            i += 1
            if i % 1000 == 0:
                print('{0}/{1}'.format(i, count))
            self._remove_duplicates(d)

    def _remove_duplicates(self, draft):
        subids = set()
        subs_to_delete = []
        ...
        for sub in subs_to_delete:
            sub.delete()
        for sub in subs_to_disassociate:
            sub.draft = None
            sub.save()
