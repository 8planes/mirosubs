...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'SubtitleRequest.action'
        db.delete_column('videos_subtitlerequest', 'action_id')

        # Adding M2M table for field actions on 'SubtitleRequest'
        db.create_table('videos_subtitlerequest_actions', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('subtitlerequest', models.ForeignKey(orm['videos.subtitlerequest'], null=False)),
            ('action', models.ForeignKey(orm['videos.action'], null=False))
        ))
        db.create_unique('videos_subtitlerequest_actions', ['subtitlerequest_id', 'action_id'])

        # Changing field 'SubtitleRequest.time'
        db.alter_column('videos_subtitlerequest', 'time', self.gf('django.db.models.fields.DateTimeField')())

        # Removing unique constraint on 'SubtitleRequest', fields ['video', 'user', 'time']
        db.delete_unique('videos_subtitlerequest', ['video_id', 'user_id', 'time'])

        # Adding unique constraint on 'SubtitleRequest', fields ['video', 'user', 'language']
        db.create_unique('videos_subtitlerequest', ['video_id', 'user_id', 'language'])
    
    
    def backwards(self, orm):
        
        # Adding field 'SubtitleRequest.action'
        db.add_column('videos_subtitlerequest', 'action', self.gf('django.db.models.fields.related.ForeignKey')(related_name='subtitlerequests', null=True, to=orm['videos.Action'], blank=True), keep_default=False)

        # Removing M2M table for field actions on 'SubtitleRequest'
        db.delete_table('videos_subtitlerequest_actions')

        # Changing field 'SubtitleRequest.time'
        db.alter_column('videos_subtitlerequest', 'time', self.gf('django.db.models.fields.DateTimeField')(auto_now=True, blank=True))

        # Adding unique constraint on 'SubtitleRequest', fields ['video', 'user', 'time']
        db.create_unique('videos_subtitlerequest', ['video_id', 'user_id', 'time'])

        # Removing unique constraint on 'SubtitleRequest', fields ['video', 'user', 'language']
        db.delete_unique('videos_subtitlerequest', ['video_id', 'user_id', 'language'])
