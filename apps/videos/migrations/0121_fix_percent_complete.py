...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        qs = orm.SubtitleLanguage.objects.filter(percent_done=0) \
            .exclude(models.Q(is_original=True)|models.Q(is_forked=True))
        ...
        for sl in qs:
            ...
            try:
                sv = sl.subtitleversion_set.order_by('-version_no')[:1].get()
            except models.ObjectDoesNotExist:
                continue
            
            if sv.subtitle_set.count():
                print 'FIXED'
                sl.percent_done = 1
                sl.save()
