...
class Migration(DataMigration):
    ...
    def _save_tvl_for_sl(self, orm, tv, sl):
        tvl = orm.TeamVideoLanguage(
            team_video=tv,
            video=tv.video,
    ...
    def _save_tvl_for_l(self, orm, tv, lang):
        tvl = orm.TeamVideoLanguage(
            team_video=tv,
            video=tv.video,
            language=lang,
    ...
    def _update_for_language(self, orm, tv, language, sublang_dict):
        if language in sublang_dict:
            sublangs = sublang_dict[language]
            for sublang in sublangs:
                self._save_tvl_for_sl(orm, tv, sublang)
        else:
            self._save_tvl_for_l(orm, tv, language)

    def update(self, orm, tv):
        orm.TeamVideoLanguage.objects.filter(team_video=tv).delete()

        sublang_dict = {}
        for sl in tv.video.subtitlelanguage_set.all():
            if sl.language in sublang_dict:
                sublang_dict[sl.language].append(sl)
            else:
                sublang_dict[sl.language] = [sl]
        for lang in settings.ALL_LANGUAGES:
            self._update_for_language(orm, tv, lang[0], sublang_dict)

    def forwards(self, orm):
        count = orm.TeamVideo.objects.count()
        i = 0
